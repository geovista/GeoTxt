 ...
 * 
 *         This class GeoCodes the input text without passing it through any NER At the moment, the goal os this class is to support the Spatial Relations Expressions project. The Solr Boosting on this is meant to give better boosting to landmarks.
 */

public class LandmarkGeoCoderAnalyzer implements Analyzer {
	...
	public NamedEntities analyze(String QueryText, NerEngines ner, Map<String, Object> context) throws IllegalArgumentException, URISyntaxException, URISyntaxException, IOException {
		...
		for (Location l : doc.locs) {
			...
			searchResult = sq.getToponymsFromSolr(l.getName(), maxRows, "landmarks");

			// filter based on relative score
			if (searchResult.size() > 0) {
				double bestScorePercentage = searchResult.get(0).getScore() * 0.01; // could
																					// make
																					// this
																					// a
																					// parameter
																					// /
																					// constant
				double bestScorePercentage2 = searchResult.get(0).getScore() * 0.1; // could
																					// make
																					// this
																					// a
																					// parameter
				...
				for (int i = 1; i < searchResult.size(); i++) {
					...
					if (score < bestScorePercentage || (score < bestScorePercentage2 && i > 5)) {
				...
				// double score = searchResult.get(i).getScore();
				//
				// System.out.print(searchResult.get(i).getName() );
				// System.out.format("%.15f%n",score);
				// }
				//
	...
	public LandmarkGeoCoderAnalyzer(int maxRows) {
		this.maxRows = maxRows;
	}

	public LandmarkGeoCoderAnalyzer() {
	...
	public static void main(String args[]) throws IllegalArgumentException, IllegalArgumentException, URISyntaxException, IOException {
		LandmarkGeoCoderAnalyzer basicStanfordAnalyizer = new LandmarkGeoCoderAnalyzer(30);
